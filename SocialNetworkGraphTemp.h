...
public:
    SocialNetworkGraph(const string& filePath) {
        Reader r(filePath);  // Use stack object, no need for new
        r.read();            // actually read and fill the network
        Network = r.getnet(); // now Network contains all users/followers/post
    }
