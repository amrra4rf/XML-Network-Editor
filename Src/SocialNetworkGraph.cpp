...
SocialNetworkGraph::SocialNetworkGraph(Graph n)
{
    Network=n;
}
Users SocialNetworkGraph::getInfluencer()
{
    ...
    if(maxFollowersId!=-1)
     return Network.IDtoUser[maxFollowersId];//return the user object of the most followed person
        ...
        return Users();//rturns and empty user object 
}
...
vector<Users>SocialNetworkGraph:: getMutualFollowers(vector<int> goal)
{
...
vector<Posts> SocialNetworkGraph::getPostsByWord(string word)
{
    vector<Posts> answer;
...
//slow algorthim
vector<Posts> SocialNetworkGraph::getPostsByTopic(string topic)
{
    vector<Posts> answer;
    for(int i=0;i<Network.users.size();i++)
    {
...
vector<Posts> SocialNetworkGraph::getPostsByTopicFastAlgorithm(string topic)
{
    vector<Posts> answer;
